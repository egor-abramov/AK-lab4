...
**Оптимизация**
  ...
  - Конструкции `<number> <math_op>`, `<identifier> @` и `<identifier> !` воспринимаются транслятором как цельные команды, что позволяет сократить количество ассемблерного кода на выходе.
